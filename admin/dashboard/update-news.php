...
if (isset($_POST['submit'])) {
    ...
    if (!empty($_FILES['news_img']['name'])) {
        ...
        move_uploaded_file($news_image_tmp, $news_image);
    } else {
    ...
    if (empty($news_name) || empty($news_description) || empty($news_date) || empty($news_time)) {
    ...
    if (empty($errors)) {
        ...
        $sql = "UPDATE `news` SET news_name=?, news_desc=?, news_date=?, news_time=?, news_image=? WHERE news_id=?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param('sssssi', $news_name, $news_description, $news_date, $news_time, $news_image, $news_id);
...
function validateForm(news) {
    ...
    const newsPrice = document.getElementById('newsPrice').value.trim();
    const newsVenue = document.getElementById('newsVenue').value.trim();
    const newsLink = document.getElementById('newsLink').value.trim();
    const newsTag = document.getElementById('newsTag').value.trim();
    ...
    if (newsImage && !validImageExtensions.includes(imageExtension)) {
        displayError('Please upload a valid image (jpg, jpeg, or png).');
        return false;
    }

    // Validate other fields not empty
    if (newsName === '' || newsDescription === '' || newsDate === '' || newsTime === '' || newsVenue === '' || newsLink === '') {
    ...
    if (newsPrice.trim() !== '') {
        ...
        if (!/^\d+(\.\d{1,2})?$/.test(newsPrice)) {
            displayError('news price must be a valid format (e.g., 10.00).');
            return false;
        }
    }
...
<script>
    function validateForm(news) {
    var newsLink = document.getElementById('newsLink').value.trim();

    if (newsLink === '') {
        displayError('Please provide the news link.');
        return false;
    }

    // Validate news link
    if (!isValidURL(newsLink)) {
        displayError('Please provide a valid URL for the news link (starting with http:// or https://).');
        return false;
    }
