...
<div class="events">
    <div class="row">
        <?php
        // Query for events
        $sql_customer = "SELECT * FROM `restos`";
        ...
        if ($result_customer && mysqli_num_rows($result_customer) > 0) {
            while ($row = mysqli_fetch_assoc($result_customer)) {
                ...
                echo '<img class="card-img-top custom-card-image" src="../admin/dashboard/' . $row['restos_img'] . '" alt="restos image">';
                echo '</div>';
                
                // Truncate the news description to a specific length (e.g., 150 characters)
                $fullDescription = $row['restos_desc'];
